...
\begin{definicao*}[Encaixados, conjunto árvore]
  ...
  Um $S \subset \mathcal{P}(V)$ é dito um conjunto árvore se quaisquer dois $A,B \in S$ (que não sejam complementares um do outro) estão encaixados.
\end{definicao*}
...
Conjuntos árvore de $\mathcal{S}(G)$ vão ser objetos mais conhecidos na literatura como decomposições em árvore. Relembrando:
...
\begin{proposicao}[Conjuntos árvore de separações $=$ decomposição em árvore]
  Seja $G$ grafo conexo. Dada uma decomposição em árvore de adesão finita $T,f$, existe um conjunto árvore $S$ de $\mathcal{S}(G)$ cuja árvore correspondente é isomorfa a $T$. Reciprocamente, todo conjunto árvore de $\mathcal{S}(G)$ vem de uma única decomposição em árvore de adesão finita.
\end{proposicao}
\begin{prova}
Seja $T,f$ uma decomposição em árvore de $G$. Denote por $V_t$ os vértices do subgrafo $f(t)$ para cada $t \in V(T)$ (as chamadas bags da decomposição). Para cada aresta $e$ de $T$ entre dois vértices $t_1$ e $t_2$, defina $X = V_{t_1} \cap V_{t_2}$. Seja $U_1$ a componente conexa de $T \setminus e$ que contém $t_1$ e $U_2$ a componente que contém $t_2$. Seja $\tilde{U}_1$ a união de todos as bags $V_t$ de $t \in U_1$ e $\tilde{U}_2$ a união de todas as bags $V_t$ de $t \in U_2$.
\end{prova}

(note que na demonstração a finitude das separações não é necessária - decomposições em árvore sem adesão finita continuam sendo conjuntos árvore, porém agora de separações infinitas)

De posse de todos estes objetos, podemos afirmar nosso principal teorema.

\begin{teorema}[Teorema da Árvores de Emaranhados]
  Em qualquer $S \subset \mathcal{P}(V)$ sub-Boole existe um conjunto árvore $S' \subset S$ que exibe todos os emaranhados de $S$.
\end{teorema}


\begin{proposicao}
  Decomposição que exibe $k$-tangles
\end{proposicao}
