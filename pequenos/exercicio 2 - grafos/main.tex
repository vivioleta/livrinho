...
\begin{proposicao*}
Existe um grafo enumerável onde cada par de vértices distintos possui um único vizinho em comum, mas nenhum vértice está ligado a todos os outros.
\end{proposicao*}

\begin{prova}
Construimos este grafo indutivamente. Suponha que no passo $i$ temos um grafo enumerável $G_i$ onde todos os pares de vértices distintos possuem ou nenhum vizinho em comum ou apenas um vizinho em comum. Suponha também que nenhum vértice está ligado a todos os outros. Adicione ao grafo $G_i$ um vértice novo para cada par de vértices distintos com nenhum vizinho em comum, e ligue este vértice apenas a este par de vértices. A cada adição de um vértice dessa forma, mantemos a propriedade de que todo par de vértices distintos possue no máximo $1$ vizinho em comum. Também mantemos a propriedade que nenhum vértice está ligado a todos os outros. Defina $G_{i+1}$ como o grafo obtido após a adição destes vértices novos para cada par de vértices de $G_i$. Ao unirmos todos os $G_i$, obteremos o grafo desejado. Começamos esta construção a partir do grafo $G_0$ com enumeráveis vértices, todos não adjacentes entre si. 
\end{prova}
