...
Dado um conjunto árvore finito $S$, sabemos que $S \cong S(T)$ para uma única árvore $T$. Cada vértice $v$ de $T$ induz uma estrela $E_v$ em $S(T)$. A imagem de $E_v$ em $S$ pelo isomorfismo é dito uma estrela-vértice do conjunto árvore $S$.
...
\begin{pergunta}
  A definição intrínseca de estrela resulta no que no caso de conjunto árvore infinitos? Raios? Isso pode indicar um possível caminho de generalização pro teorema da dualidade?
\end{pergunta}

\begin{teorema*}[Dualidade Árvore-Emaranhados]
  Seja $S \subset \mathcal{P}(V)$ fechado para complementar e finito. Seja $\mathcal{F}$ um conjunto de estrelas de $S$. Suponha que $\mathcal{F}$ satisfaz a hipótese da dualidade. Então, vale apenas uma dentre as seguintes afirmações:
\begin{itemize}[label=\ding{212}]
