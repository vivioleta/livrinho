...
Teorema da dualide árvore-emaranhados infinito: pedir que o conjunto árvore tenha todas as suas estrelas-vértice e suas extremidades em $\mathcal{F}$?
  ...
  \begin{teorema*}[Possível versão pro infinito da dualidade]
    Seja $\mathcal{F}$ um conjunto de estrelas e extremidades de $S \subset \mathcal{P}(V)$. Adicione aqui as hipóteses necessárias sobre $\mathcal{F}$ que eu vou descobrir ainda. Então:

    Existe um conjunto árvore $S' \subset S$ cujas estrelas-vértices e extremidades estão todas em $\mathcal{F}$ se e somente se todo emaranhado de $S$ seleciona uma estrela ou uma extremidade de $\mathcal{F}$
  \end{teorema*}
  \begin{prova}
