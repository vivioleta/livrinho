...
\begin{definicao*}[$S$-provas, $S$-possibilidade e $S$-consistência]
  ...
  Dizemos que $T$ $S$-prova $A$ e escrevemos $T \Sproves{S} A$ se vale pelo menos um dos itens:
    
  \begin{itemize}[label=\ding{212}]
   \item $A \in T$
    \item $B \subset A$ para um $B \in S$ onde $T \Sproves{S} A$
    \item $A = B \cap C \in S$ para $B,C \in S$ onde $T \Sproves{S} B$ e $T \Sproves{S} C$
  \end{itemize} 

  Quando dois $A,B \in S$ são tais que $A \cap B \in S$ dizemos que este é um produto ou uma interseção ou uma conjunção $S$-possível. Quando $A \subset B$ com $A,B \in S$, dizemos que $B$ é uma implicação $S$-possível. Com isso, $T \Sproves{S} A$ exatamente quando $A$ pode ser obtido em uma sequência finita de implicações e conjunções $S$-possiveis que começa com elementos de $T$. Essa sequência finita é chamada de $S$-prova.\\
...
\begin{pergunta}
  Quais são exatamente as regras lógicas para essa $S$-provabilidade, quando traduzimos de volta pro cálculo proposicional? Mais tarde mostramos um exemplo que clarifica que não é `usar dedução natural onde todos os elementos usados estão em $S$'.
\end{pergunta}
...
\begin{prova}
  Tome um perfil $\tau$ de $S$ que valora todos os elementos de $T$ em $1$. Por indução deduzimos que $\tau(A) = 1$ para todo $A \in S$ tal que $T \Sproves{S} A$. Se existisse $R \subset S$ tal que $T \cup R \Sproves{S} \varnothing$, então $\tau$ tem que valorar pelo menos um dos $A \in T$ como $0$ - logo não podemos ter $T \cup \{\lnot A\} \Sproves{S} \varnothing$ para todo $A \in R$.
\end{prova}

O teorema da completude é o que afirma a volta da proposição anterior. Infelizmente, ele não vale para nossas $S$-definições. 

Um resultado fundacional do cálculo proposicional é o Teorema da Dedução. Uma forma de o afirmá-lo é que se $T \cup \{A\} \vdash \varnothing$ então $T \vdash \lnot A$. Este resultado não vale para $S$-provas, e é o motivo que o teorema da completude não vai valer também.
...
\begin{proposicao*}[Dedução implica completude]
   Seja $\mathcal{B}$ uma álgebra de Boole, $S \subset \mathcal{B}$ um subconjunto fechado para complementar com $\varnothing \in S$ e $T \subset S$ um subconjunto qualquer.  Suponha que $S$ admite $S$-dedução. Logo, se $T$ é $S$-consistente, deve existir um perfil $\tau$ de $S$ que valora todos os elementos de $T$ em $1$.
\end{proposicao*}
\begin{prova}
...
Em cálculo proposicional, o teorema da completude implica na equivalência sintáxe-semântica $T \vdash A \iff T \vDash A$. No nosso $S$-caso, apenas provamos a ida desta equivalência, que corresponde a $S$-corretude.

\begin{proposicao*}[Equivalência sintáxe-semântica implica dedução e completude]
       Seja $\mathcal{B}$ uma álgebra de Boole, $S \subset \mathcal{B}$ um subconjunto fechado para complementar com $\varnothing \in S$.

       Suponha que para todo $T \subset S$, temos que  $T \Simplies{S} A \implies T \Sproves{S} A$. Então $S$ satisfaz $S$-dedução, e portanto também o teorema de completude.
\end{proposicao*}
\begin{prova}
Tome $T \subset S$ com $T \cup {A} \Sproves{S} \varnothing$. Tome um perfil $\tau$ que satisfaça $T$. Se $\tau(A) = 1$, teriamos que $\tau$ satisfaz $T \cup \{A\}$, e portanto por $S$-corretude, $T \cup \{A\}$ deveria ser consistente, o que já supomos que não é. Concluimos que $\tau(A) = 0$. Logo $T \Simplies{S} \lnot A$ e por hipótese temos que $T \Sproves{S} \lnot A$. Ou seja, $S$ satisfaz $S$-dedução, e já provamos anteriormente que isto implica que $S$ satisfaz o teorema de completude.
\end{prova}
...
O exemplo anterior também ilustra como a nossa noção de $S$-prova não é apenas `dedução natural onde todos os elementos usados estão em $S$'. Como $\lnot C = \lnot A \cup B$, ao traduzirmos isto de volta para lógica temos que $C$ é a implicação $A \rightarrow B$. Se usassemos modus ponens com $T = \{A, A \rightarrow B\}$, teriamos uma prova de $B$ que usa apenas elementos em $T$ - mas com a nossa noção de $S$-prova, $T$ não é capaz de provar nada fora seus próprios elementos.
...
% \end{prova}
  ...
  %Fazer a equivalência de Lindenbaum-Tarski pra álgebras de Boole. Traduzir emaranhados para esse contexto, onde vão virar `modelos $S$-consistentes'. Generalizar essa definição para outras lógicas (não precisa passar por Lindenbaum-Tarski?). Se perguntar o que vale da teoria usal agora para essas $S$-consistentes. (compacidade, completude, etc). Pensar o caso infinito. Pensar o que o teorema de árvore de emaranhados e dualidade árvore-emaranhado dizem no caso de lógica, e se tem como simplificar a prova, ou generalizar. Agora ainda mais especulativo: coloca uma relação no conjunto de modelos de uma teoria (por exemplo, a relação de ser extensão de forcing) - existem tangles nesse grafo? Brisar em lógica modal e teoria do JDH de multiverso.
