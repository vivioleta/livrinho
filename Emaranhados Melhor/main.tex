...
\begin{proposicao*}[Definição intrínseca de estrelas-vértice]
  Tome $S(T)$ com $T$ finito ou infinito, e $E \subset S(T)$ uma estrela. Então, $E = E_v$ para um vértice $v$ de $T$ se e somente se ela satisfaz a `condição de minimalidade': não existe $A \in S(T) \setminus E$ e $X \subset E$ tais que $A \subset B$ pra todo $B \in X$ e $(E \setminus X) \cup \{A\}$ é uma estrela.
\end{proposicao*}
\begin{prova}
...
\end{itemize}
  ...
  \begin{definicao*}[Extremidade de sistema de separação?]
    Seja $S \subset \mathcal{P}(V)$ fechado pra complementar, $E$ um conjunto de elementos de $S$. Dizemos que $E$ é uma extremidade se $E = \{A_1, A_2, \dots\}$ e $A_{i+1} \subset A_i$ pra todo $i$
  \end{definicao*}

      \begin{pergunta}
Teorema da dualide árvore-emaranhados infinito: pedir que o conjunto árvore tenha todas as suas estrelas-vértice e suas extremidades em $\mathcal{F}$?
  \end{pergunta}
