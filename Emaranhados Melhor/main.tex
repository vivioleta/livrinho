...
Dado uma álgebra de Boole $\mathcal{B}$ e um $S \subset \mathcal{B}$, lembramos que uma orientação $\tau$ de $S$ é um mapa de $S$ em $\{0,1\}$ tal que $\tau(A) = 0 \iff \tau(\lnot A) = 1$. Dizemos que $\tau$ satisfaz um $T \subset S$ se $\tau(t) = 1$ pra todo $t \in T$.

\begin{proposicao*}[Dualidade Árvore-Emaranhado Fraca]
Seja $\mathcal{B}$ uma álgebra de Boole e considere $\mathcal{E} \subset \mathcal{P}(\mathcal{B})$. Suponha que $S = \cup \mathcal{E}$ é finito. Chamamos os elementos de $\mathcal{E}$ de estrelas. Dizemos que duas estrelas $E_1,E_2 \in \mathcal{E}$ se colam se existe $a \in E_1, \lnot a \in E_2$. Suponha que não exista uma sequência $E_1, E_2, \dots, E_n$ tal que $E_i$ cola com $E_{i+1}$ e $E_n$ cola com $E_1$, pra $1 \leq n $. Então existe um subconjunto $T \subset \mathcal{E}$ tal que $\cup T$ é fechado pra complementar se e somente se toda orientação $\tau$ de $S = \cup \mathcal{E}$ satisfaz alguma estrela de $\mathcal{E}$.
\end{proposicao*}

\begin{prova}
Considere $\mathcal{E}$ como um grafo com arestas coloridas, onde duas estrelas $E_1$ e $E_2$ seram ligadas por uma aresta colorida por $\{a, \lnot a\} \subset S$ se elas se colam via $a$, ou seja, $a \in E_1$ e $\lnot a \in E_2$. Por simplicidade, vamos fazer referência a cor $\{a, \lnot a\}$ usando apenas um dos dois elementos, digamos $a$. A hipótese de não existência de ciclos garante que $\mathcal{E}$ é uma árvore (uma floresta, na verdade - mas o caso conexo implica o caso geral).

Chame uma estrela $E$ de triste se existe um $a \in E$ tal que $\lnot a \notin E'$ pra todo $E' \in \mathcal{E}$. Uma estrela é feliz se ela não é triste. Seja $C_E$ o conjunto de cores das arestas adjacentes a uma estrela $E$. Veja que $T \subset \mathcal{E}$ é fechado pra complementar se e somente se $T$ é uma subárvore tal que pra toda estrela $E \in T$, $E$ é feliz e $T$ contém arestas adjacentes a $E$ de todas as cores de $C_E$.
...
%Mostrar que minha versão implica a versão do Diestel

%Hipótese certa pro caso forte?

%Generalização pro infinito?
