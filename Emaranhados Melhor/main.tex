...
Geralmente dizemos que $\tau$ escolhe um $A$ ou satisfaz $A$ se $\tau(A) = 1$. Se $\tau$ escolhe todos os elementos de algum conjunto $T \subset S$ vamos dizer que $\tau$ satisfaz $T$.
...
A situação do exemplo anterior é essencialmente única: a única obstrução para um perfil não ser estendível para um ultrafiltro é ele `satisfazer uma teoria finita inconsistente' (na linguagem lógica que iremos trazer mais tarde), ou seja, valorar em $1$ uma quantidade finita de elementos com interseção vazia.

\begin{proposicao*}[Perfis que não vem de ultrafiltros satisfazem teorias inconsistentes]
Seja $\tau$ um perfil de $S \subset \mathcal{P}(V)$ que não é restrição de um ultrafiltro em $V$. Então existe um subconjunto $T \subset S$ finito tal que $\tau(t) = 1$ pra todo $t \in T$ e $\cap T = \varnothing$.
\end{proposicao*}
\begin{prova}
  Consequência direta do lema do ultrafiltro: se não existisse esse $T$, então $\tau^{-1}(1)$ satisfaria a propriedade da interseção finita e portanto estaria contido dentro de um ultrafiltro.
\end{prova}

Como na definição de perfil, pedimos consistência em relação a interseção, o único jeito possível de existir um perfil que satisfaz uma teoria inconsistente é se em $S$ `falta' as interseções necessárias para que impediriam a existência desse perfil. Em outras palavras:

\begin{proposicao*}
  Suponha que $S \subset \mathcal{P}(V)$ contenha todas as interseções possíveis de elementos em um $T \subset S$ finito. Se $\cap T = \varnothing$, então não existe perfil que satisfaz $T$.
\end{proposicao*}
\begin{prova}
  Pela propriedade de preservação do produto, $\tau$ deve valorar todas as interseções dos elementos de $T$ em $1$. Tomando, digamos, $A \in T$ e $B \in S$ a interseção de todos os outros elementos de $T$, então $\tau(A) = 1, \tau(B) = 1$ mas como $\cap T = \varnothing$ então $B \subset \lnot A$ e portanto $1 = \tau(B) \leq \tau(\lnot A) = 0$, absurdo.
\end{prova}

E, é claro, se $S$ possui todas as possíveis interseções de quaisquer elementos, então $S$ é fechado para as operações booleanas, e portanto é uma álgebra de Boole - todos os seus perfis são ultrafiltros.

Não sei o quanto essa próxima proposição é interessante. Mas, basicamente, a ideia é aumentar o conjunto $V$ criando elementos que codificam cada possível perfil de $S$. Assim, eles vão ser estendíveis a um ultrafiltro principal. É uma tentativa de `consertar' o problema da inconsistência, mas mais pra frente começo a pensar nisso como a coisa mais legal da teoria então não quero consertar nada.
...
\begin{proposicao*}
$\text{Per}(S) $ é um espaço de Stone (compacto, Hausdorff e zero dimensional).
\end{proposicao*}

\begin{prova}
  Como $S$ é fechado pra complementar e $\tau(b) = 1$ se e somente se $\tau(\lnot b) = 0$, temos que $U_b = \lnot U_{\lnot b}$. Assim, os geradores da topologia são fechados-abertos, e portanto $\text{Per}(S)$ é zero-dimensional. Também, conseguimos separar dois $\tau, \tau'$ distintos através do $U_b$ onde $b$ é um elemento tal que $\tau(b) \neq \tau'(b)$; ou seja, $\text{Per}(S)$ é Hausdorff. Finalmente, $\text{Per}(S) \subset \{0,1\}^S$ com a topologia induzida de subespaço. Como $\{0,1\}^S$ é compacto pelo teorema de Tychonoff, se provarmos que $\text{Per}(S)$ é fechado podemos concluir que ele é compacto. De fato, $\{0,1\}^S \setminus \text{Per}(S) $ é aberto pois uma função $f$ não ser um perfil deve ser testemunhado por algum par $A,B \in S$ que irá não satisfazer uma das três propriedades listadas na definição. Tomando o subconjunto aberto $U_f$ das funções que concordam com $f$ em $A,B$, vamos ter que $U_f \subset \{0,1\}^S \setminus \text{Per}(S) $. Assim, construimos uma cobertura aberta para $\{0,1\}^S \setminus \text{Per}(S) $.
\end{prova}

Podemos descrever a álgebra de Boole de $\text{Per}(S)$. A ideia aqui é criar uma álgebra de Boole que é `quase livre' sobre $S$. O `quase' vem do fato de que em $S$ já temos a operação de complementar bem definida pra todo conjunto, e também temos para alguns pares de conjuntos a operação de interseção bem definida. A álgebra que iremos construir será livre, a menos de satisfazer as operações já previamente definidas.

Vamos considerar $S \subset \mathcal{B}$ onde $\mathcal{B}$ é alguma álgebra de Boole qualquer. Relembrando, se pensarmos em $S$ apenas como um conjunto, a álgebra de Boole livre sobre $S$ é a única álgebra de Boole $F(S)$ tal que existe uma função $S \rightarrow F(S)$ e pra toda outra função $f: S \rightarrow A$ existe um homomorfismo $\tilde{f}: F(S) \rightarrow A$ que extende $f$, ou seja, que comuta o diagrama (onde $i$ é a inclusão)
% https://q.uiver.app/#q=WzAsMyxbMCwxLCJTIl0sWzEsMSwiRihTKSJdLFswLDAsIkEiXSxbMCwxLCJpIiwyLHsic3R5bGUiOnsidGFpbCI6eyJuYW1lIjoiaG9vayIsInNpZGUiOiJ0b3AifX19XSxbMCwyLCJmIl0sWzEsMiwiXFx0aWxkZXtmfSIsMl1d
\[\begin{tikzcd}
	A & \\
	S & {F(S)}
	\arrow["f", from=2-1, to=1-1]
	\arrow["i"', hook, from=2-1, to=2-2]
	\arrow["{\tilde{f}}"', from=2-2, to=1-1]
\end{tikzcd}\]

Vamos coletar as restrições induzidas por $\mathcal{B}$ em $S$ em um subconjunto $R \subset F(S)$. 

Se $A$ é o complementar de $B$ em $\mathcal{B}$, então coloque $A \cap B$ e $\lnot A \cap \lnot B$ em $R$.

Se $A$ é a interseção de dois conjuntos $B$ e $C$ em $\mathcal{B}$, então coloque $A \cap \lnot(B \cap C)$ e $\lnot A \cap (B \cap C)$ em $R$.

Seja $I$ o ideal induzido por $R$ em $F(S)$. A álgebra de Boole que procuramos será o quociente de $F(S)$ por $I$, e a denotaremos por $F_{\mathcal{B}}(S)$.

\begin{proposicao*}[A álgebra de Boole do espaço de perfis]
O espaço de ultrafiltros de $F_{\mathcal{B}}(S)$ é homeomorfo a $\text{Per}(S)$.
\end{proposicao*}
\begin{prova}
Dado um perfil $\tau: S \rightarrow \{0,1\}$, podemos usar a propriedade universal da álgebra livre pra obter um homomorfismo $\tilde{\tau}: F(S) \rightarrow \{0,1\}$. As condições de ser perfil garantem que $\tilde{\tau}$ restrita a $I$ é identicamente nula, o que garante que ela induza um homomorfismo bem definido $\tau': F_{\mathcal{B}}(S) \rightarrow \{0,1\}$, isto é, um ultrafiltro. Reciprocamente, todo homomorfismo $\tau: F_{\mathcal{B}}(S) \rightarrow \{0,1\}$ se restringe a uma função em $S$ (considerando $S \subset F_{\mathcal{B}}(S)$), que irá ser perfil devido a $\tau$ `restrita' a $I$ ser identicamente nula (em melhores palavras, que $\tau$ é uma função bem definida no quociente) (também, note aqui que preservar $\lnot$ e $\cap$ é suficiente pra concluir a outra condição de perfil, que preserva inclusão, pois $A \leq B$ se e só se $A \cap B = A$).

Construímos nossa bijeção - resta mostrar que é um homeomorfismo. Um aberto básico $\{\tau \in \text{Per}(S) \mid \tau(b) = 1\}$, com $b \in S$, é levado pela bijeção em $\{\tau \in \text{Ult}(F_{\mathcal{B}}(S)) \mid \tau(b) = 1\}$, também um aberto básico. Como $S$ gera $F_{\mathcal{B}}(S)$, estes abertos básicos formam uma sub-base para a topologia de $\text{Ult}(F_{\mathcal{B}}(S))$. Ou seja, a bijeção induz uma bijeção entre os abertos, isto é, é um homeomorfismo.
\end{prova}

%\begin{pergunta}
 % Existe alguma descrição natural a partir de $S$ da álgebra de Boole que dá origem ao espaço de Stone $\text{Per}(S)$?
%\end{pergunta}
% Sim!! :)

Uma adaptação da construção anterior também garante que existe uma álgebra de Boole pro espaço de emaranhados.
...
\begin{definicao*}[Encaixados, conjunto-árvore]
  ...
  Um $S \subset \mathcal{P}(V)$, fechado para complementar e que não contém o vazio, é dito um conjunto-árvore se quaisquer dois $A,B \in S$ estão encaixados.
\end{definicao*}

[Similar a definição de conjunto laminar - provei conversando com arjuna que todo conj árvore = fecho pra complementar de conj laminar]
...
Quando falamos árvore, queremos dizer um grafo simples e sem ciclos (não necessariamente finito). Quando falamos árvore de ordem, queremos dizer um conjunto parcialmente ordenado $T$ tal que, dado um $t \in T$, o conjunto dos elementos menores que $t$ é bem ordenado. Vamos relaxar a definição de árvore de ordem e trabalhar com a noção de ordem prefix.

\begin{definicao*}[Ordem prefix]
  Um conjunto parcialmente ordenado $T$ é dito prefix, ou uma ordem prefix, quando, dado um $t \in T$, o conjunto dos elementos menores que $t$ é uma cadeia (isto é, é totalmente ordenado).
\end{definicao*}
...
Todo prefix define um conjunto árvore. 
Defina $\lfloor v \rfloor = \{w \in T \mid v \leq w\}$ pra um $v \in T$.

\begin{proposicao}[Conjunto árvore de um prefix]
  Seja $T$ uma ordem prefix. Defina $S(T) = \{A \subset T \mid A \text{ ou } A^c \text{ é da forma } \lfloor v \rfloor \text{ para algum } v \in T \}$. Então $S(T)$ é conjunto árvore.
\end{proposicao}
\begin{prova}
...
%Aqui embaixo está uma demo errada da generalização do resultado anterior para ordens prefix. 

% Em uma ordem prefix $T$, uma cadeia inicial $C \subset T$ é um conjunto totalmente ordenado fechado pra baixo. Uma cauda de $C$ é alguma interseção $\pracima{t} \cap C$ pra algum $t \in C$
...
\begin{pergunta}
  Quem são os perfis de $S(T)$ para uma ordem prefix? Se $T$ é de ordem, acredito que provei que são as cadeias iniciais - ou seja, `vértices ou raios de $T$'.
\end{pergunta}
...
% \begin{proposicao}[Conjunto-árvores infinitos vem de ordens prefix]
...
\begin{pergunta}
  Conjunto-árvore infinito vem de ordem prefix? Acredito que a demo anterior generalizada com indução transfinita e com o resultado de perfis virem de cadeias iniciais resolveria o caso quando $S$ vem de uma árvore de ordem mas e o caso geral?
\end{pergunta}

\begin{pergunta}
Se realmente conjunto-árvore infinitos vierem de ordens prefix, esta ordem vai ser única? Imagino que não, por liberdade na escolha de raiz, mas talvez definam o mesmo espaço (ver pergunta abaixo). Existe uma noção de `isomorfismo' de árvore de ordem/ordem prefix a menos de escolha de raiz?
\end{pergunta}
...
\begin{pergunta}
  ...
  Se $S$ vem de uma árvore de ordem, acho que vão ser os espaços de ultrafiltros de tree algebras. Que no caso acho que são espaços de cadeias iniciais de árvores de ordem. (topologia induzida como subespaço do espaço de caminhos?)
\end{pergunta}
...
\begin{pergunta}
  Podemos generalizar este teorema para $S$ infinito?
\end{pergunta}
...
\begin{pergunta}
  (Pergunta do Gustavo) Dado um subconjunto aberto $U \subset \text{Per}(S)$, existe um $N \subset S$ conjunto árvore que exibe todo $\tau \in U$? E se $U$ for $G_{\delta}$?
\end{pergunta}
...
Também vai valer a pena explicitarmos a indução. Vamos definir o \textit{rank} de um $A \in T_{\vdash}$ como sendo o tamanho da menor $S-$prova de $A$ a partir de $T$. Indutivamente, comece com se $A \in T$, e diga que $\text{rank}_{T,S}(A) = 0$. Agora, para um $A \in T_{\vdash}$, se ele não foi dado um rank ainda, então: 

  \begin{itemize}[label=\ding{212}]
    \item (conjunção) se $A = B \cap C$ para $B,C \in S$ onde $T \Sproves{S} B$ e $T \Sproves{S} C$, então $\text{rank}_{T,S}(A) = \text{max}\{\text{rank}_{T,S}(B), \text{rank}_{T,S}(C)\} + 1$
    \item (implicação) se existe um $B \in S$ tal que $T \Sproves{S} B$ e vale que ou $B \subset A$ ou $T \Sproves{S} \lnot B \cup A$, então $\text{rank}_{T,S}(A) = \text{rank}_{T,S}(B) + 1$ ou $\text{rank}_{T,S}(A) = \text{max}\{\text{rank}_{T,S}(\lnot B \cup A), \text{rank}_{T,S}(B) \} + 1$
    \item (dedução) se $T \cup \{\lnot A\} \Sproves{S} \varnothing $ então $\text{rank}_{T,S}(A) = \text{rank}_{T,S}(\varnothing) + 1$
  \end{itemize}


\begin{pergunta}
  Acredito que essa definição que eu dei captura o conjunto $T_{\vdash}$ certo, porém as provas são muito rápidas, pq eu permiti acesso a relação de inclusão `$\leq$', o que é algo meio oracular. Acho que preciso dar uma definição mais computacionalmente apropriada de uma $S$-prova (que deveria ser `uma dedução natural que usa só elementos de $S$'), e definir rank apropriadamente. Nessa minha definição o rank de qualquer afirmação é sempre linear em relação a cardinalidade de $T$, o que é meio sem graça. (uma das perguntas em aberto da teoria de complexidade do cálculo proposicional é se existe um sistema de provas que é polinomial em relação ao tamanho da fórmula, por exemplo, o que mostra que nosso sistema de provas é bem mais forte do que os considerados usualmente nessa teoria).

  Um problema relacionado a escolher qual a definição certa de $S$-prova: Usando os variados sistemas axiomáticos clássicos que são equivalentes a dedução natural - se definirmos uma $S$-prova como sendo uma prova de um desses sistemas que usa apenas elementos de $S$, vamos acabar definindo o mesmo $T_{\vdash}$?Penso que não, por que para se provar equivalência desses sistemas acho que vamos precisar usar conjunções e implicações que não estão em $S$.
\end{pergunta}
...
\begin{exemplo*}[Distância da Contradição]
Em alguma álgebra de Boole $\mathcal{B}$, tome um conjunto $T \subset \mathcal{B}$ inconsistente, isto é, tal que a interseção de todos os seus elementos é $\varnothing$. Na lógica usual, não podemos ter uma semântica para este $T$, isto é, uma valoração dos elementos de $\mathcal{B}$ em $\{0,1\}$ que satisfaça $T$. Mas com perfis nós poderemos, desde que nos restrinjamos a um $S \subset \mathcal{B}$ apropriado. De fato, sabemos que $T \Sproves{\mathcal{B}} \varnothing$, então existe um número $n = \text{rank}_{T,\mathcal{B}}(\varnothing)$. Para facilitar a notação, vamos denotar o rank sem os subíndices, mas estamos sempre nos referindo ao $\text{rank}_{T,\mathcal{B}}$. Defina $S = \{A \in \mathcal{B} \mid \text{rank}(A) \leq n - 2 \text{ ou } \text{rank}(\lnot A) \leq n - 2\}$. Afirmamos que $T \subset S$ é $S$-consistente. De fato, se fosse o caso que $T \Sproves{S} \varnothing$, então devem existir $B,C \in S$ com $T \Sproves{S} B$ e $T \Sproves{S} C$ tal que $B \cap C  = \varnothing$. Mas, como $\text{rank}(B)$ e $\text{rank}(C)$ são ambos menores que $n-2$, então $\text{rank}(\varnothing)$ deveria ser menor que $n-1$, contradizendo nossa escolha de $n$. Assim, pelo teorema da $S$-completude, existe um perfil de $S$ que satisfaz $T$.\\


Podemos dar um exemplo explícito de um $T$ com $\varnothing$ de rank $n$ construindo um conjunto de $n$ conjuntos onde a interseção de todos eles é vazia, mas a interseção de qualquer número menor que $n$ é não-vazia. Pra $n = 3$, por exemplo, podemos fazer $T = \{A_1,A_2,A_3\}$ com 
 \begin{align*}
 A_1 &= \{\{1\},\{1,2\},\{1,3\}\}                       \\
 A_2 &= \{\{2\}, \{1,2\}, \{2,3\} \}          \\
 A_3 &= \{\{3\},\{2,3\}, \{1,3\}\}          
 \end{align*}
e $T \subset \mathcal{B} = \mathcal{P}(\mathcal{P}(\{1,2,3\}))$. Esta notação esquisita vem da forma como vamos construir o caso geral. Tome o conjunto $V = \mathcal{P}(\{1,2,\dots, n\})$ e $\mathcal{B} = \mathcal{P}(V)$. Faça $A_i = \{A \in V \mid |A| < n \text{ e } i \in A \} \in \mathcal{B}$. Então $T = \{A_1, \dots, A_n\} \subset \mathcal{B}$ é tal que a interseção de qualquer número menor que $n$ é não-vazia, mas a interseção de todos eles é vazia.\\

Não vou me preocupar muito em escrever agora as propriedades de rank que usei nos últimos paragrafos para justificar certas afirmações porque acho que vou ter que mudar a noção de rank para ser mais próxima da noção de complexidade de prova na teoria de complexidade de prova usual. Me aproximando mais disso, posso usar exemplos como a sentença de Godel ou o Pidgeon-Hole Principle onde foi provado que suas demonstrações devem ter um tamanho grande. Ver https://mathoverflow.net/questions/149115/deep-theorems-and-long-proofs/
\end{exemplo*}
...
Vamos tentar agora trazer o conceito de tangles pra lógica de primeira ordem. Uma ideia é usar a semântica de `boolean-valued models', mas conversando com o Gustavo acho que talvez nem preciso disso - é só pedir que as valorações satisfaçam a consistência com os operadores pedindo que se ela valora $\phi(a)$ como $1$ então valora $\exists x \phi(x)$ como $1$.
...
%  Ideia 1:  S = n-cláusulas, tangles de S não tem 2n-contradições. como tangles se relacionam com complexidade computacional? 'distancia da contradição'

% Ideia 2: tangle como cluster no multiverso de modelos
